#!/usr/bin/env node
/**
 * Esvazia WHATSAPP_INGEST_PHONE_ALLOWLIST no serviço agente_sumare (EasyPanel)
 * para liberar o atendimento a TODOS os números (fim da fase de teste).
 * Altera SOMENTE essa linha; preserva o restante do env. Depois redeploya.
 *
 * Use `--dry` para só mostrar o diff sem aplicar.
 */
...
const KEY = 'WHATSAPP_INGEST_PHONE_ALLOWLIST'
...
if (!found) {
  console.error(`Chave ${KEY} não encontrada no env — abortando por segurança.`)
  process.exit(1)
}
...
// Tenta updateEnv; cai pra update completo se necessário.
