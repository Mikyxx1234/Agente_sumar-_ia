...
async function fetchAllCursos({ url, key }) {
  // PostgREST devolve até 1000 por padrão; mais que isso precisaria
  // paginar com Range. 277 cabe num só.
...
/**
 * Roda o reindex completo.
 *
 * @param {Record<string,string>} env
 * @param {{ clear?: boolean }} [opts]
 *   clear: true → apaga tudo antes (pra evitar duplicatas em re-runs).
 *                Default true.
 * @returns {Promise<{ ok: boolean, total: number, batches: number, durationMs: number,
...
export async function reindexPos(env, opts = {}) {
  ...
  const model = resolveModel(env, 'embeddings') // text-embedding-3-small
  const clear = opts.clear !== false

  const cursos = await fetchAllCursos(cfg)
  if (!Array.isArray(cursos) || cursos.length === 0) {
    return { ok: false, total: 0, batches: 0, error: 'Tabela cursos_salesbot_pos vazia.', durationMs: Date.now() - t0, model }
  }

  if (clear) {
    await clearVectorTable(cfg)
  }
  ...
  for (let i = 0; i < cursos.length; i += BATCH) {
    ...
    const texts = slice.map((c) => c.Curso || '').map((s) => String(s).trim())
